...
public partial class TextEncryptionForm : Form
{
    ...
    public TextEncryptionForm(IServiceProvider serviceProvider)
    {
    ...
    {
        Text = "تشفير النصوص";
        lblTitle.Text = "تشفير النصوص";
        
        txtInputText.PlaceholderText = "أدخل النص المراد تشفيره هنا...";
        txtOutputText.PlaceholderText = "سيظهر النص المشفر هنا...";
        txtPassword.PlaceholderText = "أدخل كلمة مرور قوية";
        txtConfirmPassword.PlaceholderText = "أعد إدخال كلمة المرور";
        ...
        toolTip.SetToolTip(btnEncrypt, "تشفير النص المدخل");
        toolTip.SetToolTip(btnCopyInput, "نسخ النص الأصلي إلى الحافظة");
        toolTip.SetToolTip(btnCopyOutput, "نسخ النص المشفر إلى الحافظة");
        toolTip.SetToolTip(btnPasteInput, "لصق النص من الحافظة");
        toolTip.SetToolTip(btnClear, "مسح جميع الحقول");
        toolTip.SetToolTip(btnGeneratePassword, "توليد كلمة مرور آمنة");
        toolTip.SetToolTip(chkAutoClearClipboard, "مسح الحافظة تلقائياً بعد فترة من الوقت");
        
        UpdateFieldsState();
        
        _logger.LogInfo("TextEncryptionForm", "تم تهيئة نموذج تشفير النصوص");
    }
    ...
    private async Task ProcessTextAsync()
    {
        try
        {
            ...
            OperationResult result = await _textEncryptionService.EncryptTextAsync(
            ...
            if (result.Success)
            {
                ...
                {
                    txtOutputText.Text = "تم تشفير النص بنجاح!";
                }
                
                MessageBox.Show(result.Message, "تم بنجاح", 
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                
                _logger.LogInfo("TextEncryptionForm", 
                    $"تم تشفير النص بنجاح، الطول: {inputText.Length}");
                
                AddToHistory(inputText, txtOutputText.Text, true, result);
            }
            else
            {
                ...
                _logger.LogError("TextEncryptionForm", 
                    $"فشل في تشفير النص", 
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError("TextEncryptionForm", "خطأ غير متوقع في معالجة النص", ex);
        }
        finally
        {
    ...
    private void BtnCopyInput_Click(object? sender, EventArgs e)
    {
        if (!string.IsNullOrWhiteSpace(txtInputText.Text))
        {
            ...
            MessageBox.Show("تم نسخ النص الأصلي إلى الحافظة", "تم", 
                MessageBoxButtons.OK, MessageBoxIcon.Information);
            
            StartClipboardTimer();
            
            _logger.LogInfo("TextEncryptionForm", "تم نسخ النص الأصلي إلى الحافظة");
        }
    }
    ...
    private void BtnCopyOutput_Click(object? sender, EventArgs e)
    {
        if (!string.IsNullOrWhiteSpace(txtOutputText.Text))
        {
            ...
            _logger.LogInfo("TextEncryptionForm", "تم نسخ النص المشفر إلى الحافظة");
        }
    }
    ...
    private void BtnPasteInput_Click(object? sender, EventArgs e)
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(clipboardText))
            {
                ...
                _logger.LogInfo("TextEncryptionForm", "تم لصق النص من الحافظة");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError("TextEncryptionForm", "فشل في لصق النص من الحافظة", ex);
    ...
    private void BtnClear_Click(object? sender, EventArgs e)
    {
        ...
        if (result == DialogResult.Yes)
        {
            ...
            _logger.LogInfo("TextEncryptionForm", "تم مسح جميع الحقول");
        }
    }
    ...
    private void BtnGeneratePassword_Click(object? sender, EventArgs e)
    {
        try
        {
            var passwordGenerator = _serviceProvider.GetRequiredService<PasswordGenerator>();
            ...
            MessageBox.Show("تم توليد كلمة مرور آمنة", "تم", 
                MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        catch (Exception ex)
        {
            _logger.LogError("TextEncryptionForm", "فشل في توليد كلمة المرور", ex);
            MessageBox.Show("تعذر توليد كلمة المرور", "خطأ", 
                MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    ...
    private bool ValidateInputs()
    {
        if (string.IsNullOrWhiteSpace(txtInputText.Text))
        {
            MessageBox.Show("يجب إدخال نص للمعالجة", "خطأ", 
                MessageBoxButtons.OK, MessageBoxIcon.Error);
            txtInputText.Focus();
            return false;
        }
        ...
        {
            MessageBox.Show("كلمة المرور يجب أن تكون 8 أحرف على الأقل", "خطأ", 
                MessageBoxButtons.OK, MessageBoxIcon.Error);
            txtPassword.Focus();
            return false;
        }
        ...
        {
            MessageBox.Show("كلمة المرور وتأكيدها غير متطابقين", "خطأ", 
                MessageBoxButtons.OK, MessageBoxIcon.Error);
    ...
    private void UpdateFieldsState()
    {
        ...
        btnEncrypt.Enabled = hasInput && hasPassword;
    ...
    private void txtPassword_TextChanged(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrWhiteSpace(txtPassword.Text))
        {
            ...
            {
                var passwordGenerator = _serviceProvider.GetRequiredService<PasswordGenerator>();
                var strength = passwordGenerator.EvaluateStrength(txtPassword.Text);
                
                progressPasswordStrength.Value = strength switch
                {
                ...
                lblPasswordStrength.Text = $"قوة كلمة المرور: {passwordGenerator.GetStrengthDescription(strength)}";
                
                progressPasswordStrength.ForeColor = strength switch
                {
            ...
            {
                _logger.LogError("TextEncryptionForm", "فشل في تقييم قوة كلمة المرور", ex);
            }
        }
        else
        {
            ...
            progressPasswordStrength.ForeColor = Color.Gray;
        }
    }
    ...
    private void btnAnalyzeText_Click(object sender, EventArgs e)
    {
        if (!string.IsNullOrWhiteSpace(txtInputText.Text))
        {
            ...
            string analysis = $"تحليل النص:\n" +
                            $"طول النص: {length} حرف\n" +
                            $"عدد السطور: {lines}\n" +
                            $"عدد الكلمات: {words}\n" +
                            $"نسبة الأحرف العربية: {CalculateArabicPercentage(text):F1}%\n" +
                            $"نسبة الأحرف الإنجليزية: {CalculateEnglishPercentage(text):F1}%";
            
            MessageBox.Show(analysis, "تحليل النص", 
                MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
    }
    
    private double CalculateArabicPercentage(string text)
    {
        if (string.IsNullOrEmpty(text))
            return 0;
        
        int arabicCount = text.Count(c => c >= 0x0600 && c <= 0x06FF);
        return (double)arabicCount / text.Length * 100;
    }
    
    private double CalculateEnglishPercentage(string text)
    {
        if (string.IsNullOrEmpty(text))
            return 0;
        
        int englishCount = text.Count(c => (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z'));
        return (double)englishCount / text.Length * 100;
    }
}