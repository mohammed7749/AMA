...
public partial class FileEncryptionForm : Form
{
    ...
    public FileEncryptionForm(IServiceProvider serviceProvider)
    {
    ...
    private void InitializeForm()
    {
        ...
        chkAutoDelete.Checked = _settings.AutoDeleteOriginal;
        ...
        toolTip.SetToolTip(btnEncrypt, "تشفير الملف أو المجلد المحدد");
        ...
        toolTip.SetToolTip(chkAutoDelete, "تحذير: هذا الإجراء لا يمكن التراجع عنه!");
        ...
        _logger.LogInfo("FileEncryptionForm", "تم تهيئة نموذج تشفير الملفات");
    }
            ...
            try
            {
                await Task.Run(async () =>
                {
                    ...
                    Invoke((MethodInvoker)(() =>
                    {
                        ...
                        {
                            AutoGenerateOutputPath(dialog.FileName, true);
                        }
                    }));
    ...
    private void BtnBrowseOutput_Click(object? sender, EventArgs e)
    {
        ...
        {
            Title = "حدد موقع حفظ الملف المشفر",
    ...
    private void AutoGenerateOutputPath(string inputPath, bool encrypt)
    {
        ...
        else
        {
            if (inputPath.EndsWith(".encrypted", StringComparison.OrdinalIgnoreCase))
            {
                outputPath = Path.Combine(
                    Path.GetDirectoryName(inputPath)!,
                    Path.GetFileNameWithoutExtension(Path.GetFileNameWithoutExtension(inputPath)) + "_decrypted" + Path.GetExtension(inputPath));
            }
            else
            {
                outputPath = Path.Combine(
    ...
    private async Task ProcessFileAsync()
    {
        try
        {
            ...
            lblFileInfo.Text = "جاري تشفير الملف...";
            
            OperationResult result = await _fileEncryptionService.EncryptFileAsync(
                inputPath, outputPath, password, username, additionalKey, 
                progress, cancellationToken);
            
            if (result.Success)
            {
                lblFileInfo.Text = "تم تشفير الملف بنجاح!";
                
                MessageBox.Show(result.Message, "تم بنجاح", 
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                
                _logger.LogInfo("FileEncryptionForm", 
                    $"تم تشفير الملف بنجاح: {inputPath}");
                
                AddToProcessedFiles(inputPath, outputPath, true, result);
            ...
            else
            {
                ...
                _logger.LogError("FileEncryptionForm", 
                    $"فشل في تشفير الملف: {inputPath}", 
        ...
        catch (OperationCanceledException)
        {
            ...
            _logger.LogWarning("FileEncryptionForm", "تم إلغاء عملية التشفير");
        }
        catch (Exception ex)
        {
            ...
            _logger.LogError("FileEncryptionForm", "خطأ غير متوقع في معالجة الملف", ex);
        }
        finally
        {
    ...
    private void SetProcessingState(bool processing)
    {
        ...
        chkAutoDelete.Enabled = !processing;
    ...
    private void BtnClear_Click(object? sender, EventArgs e)
    {
        ...
        if (result == DialogResult.Yes)
        {
            ...
            _logger.LogInfo("FileEncryptionForm", "تم مسح جميع الحقول");
        }
    }
    ...
    private void BtnGeneratePassword_Click(object? sender, EventArgs e)
    {
        ...
        {
            _logger.LogError("FileEncryptionForm", "فشل في توليد كلمة المرور", ex);
    ...
    private void BtnOpenInput_Click(object? sender, EventArgs e)
    {
        if (!string.IsNullOrWhiteSpace(txtInputPath.Text) && File.Exists(txtInputPath.Text))
        {
            ...
            {
                _logger.LogError("FileEncryptionForm", "فشل في فتح الملف", ex);
    ...
    private bool ValidateInputs()
    {
        ...
        {
            MessageBox.Show("يجب تحديد ملف أو مجلد للإدخال", "خطأ", 
                MessageBoxButtons.OK, MessageBoxIcon.Error);
            txtInputPath.Focus();
            return false;
        }
    ...
    private async Task LoadFileMetadataAsync(string filePath)
    {
        try
        {
            ...
            await Task.Run(async () =>
            {
                ...
                {
                    _logger.LogError("FileEncryptionForm", "فشل في تحليل بيانات الملف", ex);
                }
            });
        }
        catch (Exception ex)
    ...
    private void UpdateFieldsState()
    {
        ...
        btnEncrypt.Enabled = hasInput && hasPassword && !_isProcessing;
    ...
    private void txtPassword_TextChanged(object sender, EventArgs e)
    {
        ...
        if (!string.IsNullOrWhiteSpace(txtPassword.Text))
        {
            ...
            {
                _logger.LogError("FileEncryptionForm", "فشل في تقييم قوة كلمة المرور", ex);
            }
        }
        else
        {
